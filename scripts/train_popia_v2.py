...
def onnx_macro_f1(onnx_path, tokenizer, eval_rows: list[dict], batch_size: int = 16) -> tuple[float, int]:
...
def main() -> int:
    ...
    variants = {
        ...
        "model_qint8_avx512_vnni.onnx": AutoQuantizationConfig.avx512_vnni(is_static=False, per_channel=True),
    ...
    for filename in variants:
        q_v1_f1, q_v1_distinct = onnx_macro_f1(onnx_dir / filename, tokenizer, v1_rows, args.batch_size)
        q_v2_f1, q_v2_distinct = onnx_macro_f1(onnx_dir / filename, tokenizer, v2_rows, args.batch_size)
        print(
        ...
        if q_v1_f1 < trained_v1_f1 - ARTIFACT_TOL or q_v2_f1 < trained_v2_f1 - ARTIFACT_TOL:
            sys.exit(f"ARTIFACT GATE FAIL: {filename} macro F1 regressed >{ARTIFACT_TOL} from PyTorch — not shipping")
