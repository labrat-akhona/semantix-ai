"""Expand GDPR seed NLI pairs into a training set via LLM paraphrase.

Reads data/gdpr_seeds.jsonl, calls an LLM to produce ~30 variants per seed
preserving the label, deduplicates, and writes data/gdpr_train.jsonl.

Usage:
    OPENAI_API_KEY=... python scripts/expand_gdpr_seeds.py
    # or
    ANTHROPIC_API_KEY=... python scripts/expand_gdpr_seeds.py --provider anthropic
"""
...
- vary EU member state context where plausible (Ireland, Germany, France,
  Netherlands, Spain, Italy, Poland, etc.) and the relevant supervisory
  authority (DPC, CNIL, BfDI, AEPD, Garante, Autoriteit Persoonsgegevens, etc.)
- vary phrasing register (casual, formal, legalese)
- keep the NLI label identical to the original

The clause (GDPR article/concept) is: {clause}
