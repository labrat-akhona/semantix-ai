"""Fine-tune cross-encoder/nli-MiniLM2-L6-H768 on GDPR data, export to ONNX.

Usage:
    python scripts/train_gdpr.py           # GPU if available, else CPU
    python scripts/train_gdpr.py --epochs 5

Requires: pip install semantix-ai[train]

Prerequisites (v0.3 roadmap — not yet landed):
    1. Grow data/gdpr_seeds.jsonl beyond the 21 hand-authored pairs.
    2. Author data/gdpr_eval.jsonl (target: 150 hash-pinned holdout pairs,
       mirroring scripts/_popia_eval_hash.txt discipline).
    3. Run scripts/expand_gdpr_seeds.py to produce data/gdpr_train.jsonl.
    4. Run this script. The output checkpoint uploads to
       https://huggingface.co/labrat-aiko/nli-gdpr-v1 .
"""
...
TRAIN_PATH = Path("data/gdpr_train.jsonl")
SEEDS_PATH = Path("data/gdpr_seeds.jsonl")
EVAL_PATH = Path("data/gdpr_eval.jsonl")
EVAL_HASH_PATH = Path("scripts/_gdpr_eval_hash.txt")
OUT_DIR = Path("out/nli-gdpr-v1")
...
def verify_eval_integrity() -> None:
    ...
    if not EVAL_HASH_PATH.exists():
        sys.exit(f"missing {EVAL_HASH_PATH} -- author data/gdpr_eval.jsonl and pin its SHA-256 first")
...
def main() -> int:
    ...
    print(f"\nDONE. Upload {OUT_DIR} to HuggingFace as labrat-aiko/nli-gdpr-v1")
    return 0
