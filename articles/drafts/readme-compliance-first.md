# README rewrite proposal — compliance-first positioning

**Status:** proposal for review. Not yet swapped into the live README. The current README leads with the pytest use case; this draft leads with the audit/regulated-industry story while keeping every factual claim verifiable and every existing use case documented.

**Why change:** Earlier strategy work identified the audit-trail + determinism combo as semantix's defensible moat against LLM-as-judge and against competitors like TruLens and DeepEval. The current README buries the audit engine in "Advanced Features" at the bottom. A compliance-first reader bounces before they see it.

**What stays the same:** every code example, every install command, every integration, every honest claim (248 tests, ~15 ms, 21% → 70% retry reliability). No features added or removed.

**Diff summary:**
- Tagline changes
- First screen reframed from "test your chatbot" to "validate, log, prove"
- Audit trail promoted from Advanced Features to Primary Feature
- Testing example kept but moved to the "use cases" section
- POPIA / EU AI Act explicitly named once, in context, not as marketing spin
- Footer preserved
...
Validate every LLM output against an explicit intent. Get back a score, a verdict, and a tamper-evident receipt. Locally. In under 50 milliseconds. Without an API key.
...
<details>
<summary><strong>DSPy</strong> (click to expand)</summary>
...
## Things to decide before swapping in

1. **POPIA / EU AI Act mention in the README?** The draft names both once, in context, in the "Why this exists" section. This lands for regulated-industry readers but could read as heavy-handed for a generalist reader. Alternatives: keep only in `docs/` and link from README; mention only POPIA (home-market focus); drop both and keep the pitch abstract ("your regulator, your auditor, your compliance team").

2. **Opening example — decorator or testing?** This draft leads with `@validate_intent(ResolutionPolite, audit=True)` (the audit-aware decorator). That's the aligned example for the new positioning. The current README leads with `assert_semantic` which is more approachable for first-time Python devs. We could keep the testing example first and move the audit example to "What you get" → "1. Validation as a decorator."

3. **Collapsing the integrations.** Draft puts each framework in a `<details>` block. This shortens the first-scroll view but hides the DSPy example (which is semantix's single biggest current distribution channel). Alternative: keep DSPy expanded, collapse the rest.

4. **Drop the "Self-Training Flywheel" section?** The current README has it. The draft removed it for space — the training story is real but it's a second-order feature and dilutes the compliance-first frame. Decision: keep (current position, expanded) / keep (moved to docs only) / drop from README, expand in docs.

5. **Factual claim verification.** Before swapping in, confirm each claim is still accurate:
   - "248 tests" — verify current test count with `pytest --collect-only -q | wc -l`
   - "21% to 70% across 3 intent categories" — this is from an older benchmark; confirm the source
   - "~15 ms per check" — our own fast-fail benchmark measured 51 ms avg on this machine. Decide whether to keep "~15 ms" (which is the quantized model's core inference) or quote the end-to-end number we measured. Current docs/integrations/dspy.md says "~15 ms"; both the README and docs should match whatever we settle on.

## Swap-in procedure when approved

```bash
cp README.md README.md.bak  # keep the current README for reference
# Edit README.md with the approved version of the draft above
git add README.md articles/drafts/readme-compliance-first.md
git commit -m "docs: compliance-first README — lead with audit/determinism positioning"
```
